...
#define MAXIMUM_FINAL_DAMAGE 60
...
//	req_access = list(ACCESS_CLONING) //FOR PREMATURE UNLOCKING.
	...
	var/final_damage //The clone is released once its damage reaches this level.
...
/obj/machinery/clonepod/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += "<span class='notice'>The status display reads: Cloning speed at <b>[speed_coeff*50]%</b>.<br>Predicted amount of cellular damage: <b>[final_damage]%</b>.</span>"
		if(efficiency > 5)
...
/obj/machinery/clonepod/proc/get_completion()
	...
	var/mob/living/mob_occupant = occupant
	if(mob_occupant)
		. = (100 * ((mob_occupant.health + 100) / (200 - final_damage)))
...
/obj/machinery/clonepod/proc/growclone(clonename, ui, mutation_index, mindref, last_death, blood_type, datum/species/mrace, list/features, factions, list/quirks, datum/bank_account/insurance, list/traumas, empty)
	...
	var/mob/living/carbon/human/new_clone = new /mob/living/carbon/human(src)

	if(!clonename)	//to prevent null names
		clonename = "clone ([rand(1,999)])"
	new_clone.real_name = clonename

	new_clone.hardset_dna(ui, mutation_index, new_clone.real_name, blood_type, mrace, features)

	if(new_clone.can_mutate())//dont apply mutations if the species is Mutation proof.
		if(efficiency > 2)
			var/list/unclean_mutations = (GLOB.not_good_mutations|GLOB.bad_mutations)
			new_clone.dna.remove_mutation_group(unclean_mutations)
		if(efficiency > 5 && prob(20))
			new_clone.easy_random_mutate(POSITIVE)
		if(efficiency < 3 && prob(50))
			new_clone.easy_random_mutate(NEGATIVE+MINOR_NEGATIVE)

	new_clone.silent = 20 //Prevents an extreme edge case where clones could speak if they said something at exactly the right moment.
	occupant = new_clone

	icon_state = "pod_1"
	//Get the clone body ready
	maim_clone(new_clone)
	ADD_TRAIT(new_clone, TRAIT_STABLEHEART, CLONING_POD_TRAIT)
	ADD_TRAIT(new_clone, TRAIT_STABLELIVER, CLONING_POD_TRAIT)
	ADD_TRAIT(new_clone, TRAIT_EMOTEMUTE, CLONING_POD_TRAIT)
	ADD_TRAIT(new_clone, TRAIT_MUTE, CLONING_POD_TRAIT)
	ADD_TRAIT(new_clone, TRAIT_NOBREATH, CLONING_POD_TRAIT)
	ADD_TRAIT(new_clone, TRAIT_NOCRITDAMAGE, CLONING_POD_TRAIT)
	new_clone.Unconscious(80)

	if(new_clone)
		new_clone.faction |= factions

		for(var/quirk in quirks)
			var/datum/quirk/cloned_quirk = new quirk(new_clone)
			cloned_quirk.on_clone(quirks[quirk])

		for(var/t in traumas)
			var/datum/brain_trauma/BT = t
			var/datum/brain_trauma/cloned_trauma = BT.on_clone()
			if(cloned_trauma)
				new_clone.gain_trauma(cloned_trauma, BT.resilience)

		new_clone.set_cloned_appearance()

		new_clone.set_suicide(FALSE)
	attempting = FALSE
...
/obj/machinery/clonepod/process()
	...
	if(!is_operational()) //Autoeject if power is lost
		...
		if(mob_occupant && (mob_occupant.stat == DEAD) || (mob_occupant.suiciding))  //Autoeject corpses and suiciding dudes.
		...
		else if(mob_occupant && mob_occupant.getToxLoss() > final_damage)
			mob_occupant.Unconscious(80)
			var/dmg_mult = CONFIG_GET(number/damage_multiplier)
			 //Slowly get that clone healed and finished.
			mob_occupant.adjustToxLoss(-((speed_coeff / 2) * dmg_mult), forced = TRUE)
			var/installed = flesh_number - unattached_flesh.len
			var/progress = min(1, (CLONE_INITIAL_DAMAGE - mob_occupant.getToxLoss()) / (CLONE_INITIAL_DAMAGE - final_damage))
			...
			for(var/flesh in unattached_flesh)
				attach_flesh(mob_occupant, flesh)
...
/obj/machinery/clonepod/attach_flesh(mob/clone, obj/item/flesh)
	// attach some flesh
	if(isorgan(flesh))
		var/obj/item/organ/organ = flesh
		organ.organ_flags &= ~ORGAN_FROZEN
		organ.Insert(clone)
	else if(isbodypart(flesh))
		var/obj/item/bodypart/bodypart = flesh
		bodypart.attach_limb(clone)
...
/obj/machinery/clonepod/proc/go_out()
	...
	icon_state = "pod_0"
	mob_occupant.domutcheck() //Waiting until they're out before possible monkeyizing. The 1 argument forces powers to manifest.
...
/obj/machinery/clonepod/proc/maim_clone(mob/living/carbon/human/H)
	...
	H.setToxLoss(CLONE_INITIAL_DAMAGE, forced = TRUE)     //Yeah, clones start with very low health, not with random, because why would they start with random health
