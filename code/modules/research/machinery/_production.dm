...
/obj/machinery/production/Initialize(mapload)
	. = ..()
...
/obj/machinery/production/rnd/proc/on_connected_techweb()
	RegisterSignals(
		stored_research,
...
/obj/machinery/production/rnd/Destroy()
	cached_designs = null
	return ..()

/obj/machinery/production/rnd/proc/on_techweb_update()
	SIGNAL_HANDLER
...
/obj/machinery/production/rnd/proc/update_designs()
...
/obj/machinery/production/RefreshParts()
	. = ..()
...
/obj/machinery/production/ui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "Fabricator")
		ui.open()

/obj/machinery/production/ui_static_data(mob/user)
...
/obj/machinery/production/ui_data(mob/user)
	var/list/data = list()
...
/obj/machinery/production/rnd/ui_act(action, list/params, datum/tgui/ui)
	. = ..()
...
/obj/machinery/production/proc/calculate_efficiency()

/obj/machinery/production/rnd/calculate_efficiency()
	efficiency_coeff = 1
...
/obj/machinery/production/proc/user_try_print_id(mob/user, design_id, print_quantity)
	...
	var/coefficient = build_efficiency(design.material_efficiency_affected)
...
/obj/machinery/production/proc/start_making(datum/design/design, build_count, build_time_per_item, build_efficiency, charge_per_item)
	PROTECTED_PROC(TRUE)
...
/obj/machinery/production/proc/do_make_item(datum/design/design, build_efficiency, time_per_item, charge_per_item, items_remaining)
	PROTECTED_PROC(TRUE)
	...
	if(!items_remaining)
		finalize_build()
		return
	addtimer(CALLBACK(src, PROC_REF(do_make_item), design, build_efficiency, time_per_item, charge_per_item, items_remaining), time_per_item)

/// Resets the busy flag
/// Called at the end of do_make_item's timer loop
/obj/machinery/production/proc/finalize_build()
	PROTECTED_PROC(TRUE)
	busy = FALSE
	update_static_data_for_all_viewers()

// Stuff for the stripe on the department machines
/obj/machinery/production/default_deconstruction_screwdriver(mob/user, icon_state_open, icon_state_closed, obj/item/screwdriver)
	. = ..()

	update_icon(UPDATE_OVERLAYS)

/obj/machinery/production/update_overlays()
	. = ..()
...
/obj/machinery/production/proc/reset(wire)
	switch(wire)
		if(WIRE_HACK)
...
/obj/machinery/production/proc/shock(mob/user, prb)
...
/obj/machinery/production/proc/adjust_hacked(state)
	hacked = state
	update_static_data_for_all_viewers()
